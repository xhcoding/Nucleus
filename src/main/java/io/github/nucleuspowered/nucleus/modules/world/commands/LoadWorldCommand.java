...
public class LoadWorldCommand extends io.github.nucleuspowered.nucleus.internal.command.AbstractCommand<CommandSource> {
    ...
    public CommandResult executeCommand(final CommandSource src, CommandContext args) throws Exception {
        ...
        src.sendMessage(plugin.getMessageProvider().getTextMessageWithFormat("command.world.load.begin", worldProperties.getWorldName()));
        if (Sponge.getServer().getUnloadedWorlds().stream().anyMatch(x -> x.getUniqueId().equals(worldProperties.getUniqueId()))) {
            ...
            src.sendMessage(plugin.getMessageProvider().getTextMessageWithFormat("command.world.load.success", worldProperties.getWorldName()));
            return CommandResult.success();
        }

        src.sendMessage(plugin.getMessageProvider().getTextMessageWithFormat("command.world.load.alreadyloaded", worldProperties.getWorldName()));
